...
from Dataset import get_trainloader_single,get_valloader_single,get_combined_dataset,get_trainloader_CNN_single,get_testloader_CNN_single,get_CNN_dataset
...
datasets = ['1', '2', '3']
source='saved'
pth_root='model_params_from_cv2'
pth_model='Transformer-NoTimeEncode'
...
def evaluate_CNN(model,val_loader):
    ...
    with torch.no_grad():
        for features, labels,times in val_loader:
            features = features.to(device)
            times = times.to(device)
            labels = labels.to(device)
            ...
            # 计算预测类别
            _, predicted = torch.max(outputs.data, 1)  # 在 num_classes 维度上取最大值的索引，即预测类别

            # 计算真实类别
            single_labels = torch.argmax(labels, dim=1)  # 在 num_classes 维度上取最大值的索引，即真实类别

            # 将每个批次的有效预测结果和真实标签添加到列表中
            all_valid_predicted.append(predicted.cpu().numpy())
            all_valid_single_labels.append(single_labels.cpu().numpy())
...
def test_model():
    ...
    for dataset in datasets:
        ...
        combined_data.append(get_combined_dataset(data_mat, best_seq_len))
    ...
    #test_loader=get_testloader_CNN_single(test_mat, batch_size)
    test_loader = get_valloader_single(test_mat, batch_size, best_seq_len)

    # Initialize model with the specified hyperparameters
    model = get_trans_model(feature_dim=96, num_types=num_classes, d_model=256, d_inner=1024, 
                           n_layers=4, n_head=4, d_k=64, d_v=64, dropout=0.1).to(device)
    ...
    if source=='saved':
        pth_path=f'{pth_root}/{pth_model}/{subject}_cv2_model_parameters.pth'
        ...
        #train_CNNmodel(model, train_loader, criterion, optimizer, best_num_epochs)
        print("Training completed.")

        # Save the model parameters
        print("Saving model parameters...")
        torch.save(model.state_dict(), f"{subject}_cv2_model_parameters.pth")
        #torch.save(model.state_dict(), f"{subject}_cv2_CNNmodel_parameters.pth")
    ...
    #true_labels, predicted_labels= evaluate_CNN(model, test_loader)
